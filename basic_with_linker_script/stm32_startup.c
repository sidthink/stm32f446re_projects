...
void Reset_Handler(void); //__attribute__((section(.random)));
...
//any variable created by defualt is placed in .data or .bss section in obj file.
// To force the compiler to place a variable in a separate section -> "data type var_name __attribute__((section("section_name")))"
...
};//or __attribute__((section(.isr_vector)))

void Reset_Handler()
{
    //copy .data section to SRAM
    uint32_t size = (uint32_t)&_edata -(uint32_t)&_sdata;  // address value 
    //printf( "_edata = %ln, _sdata= %ln\n", &_edata, &_sdata);
     uint32_t size1 = &_edata - &_sdata; 
     //printf("Size : %ld , _edata = %ln, _sdata= %ln\n", size, &_edata, &_sdata);
